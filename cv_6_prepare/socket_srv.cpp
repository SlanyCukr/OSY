        ...
        if(fork() == 0)
        {
            ...
            { 
                ...
                if ( l_read_poll[ 1 ].revents & POLLIN )
                {
                    ...
                    // zjistime GET /parametr
                    char l_buf_copy[5000];
                    strcpy(l_buf_copy, l_buf);
                    char *ptr = strtok(l_buf_copy, " ");
                    ptr = strtok(NULL, " ");
                    ptr = ptr + 1;

                    // VYTVORENI NOVEHO POTOMKA, ROURA, EXEC
                    char html_body_buf[ 1000 ];
                    ...
                    if (pid == 0) {
                        ...
                        execlp(ptr, ptr, nullptr); 
                    }
                    if (pid > 0) {
                        // parent
                        close(roura[1]);

                        int l_len = read( roura[ 0 ], html_body_buf, sizeof( html_body_buf ) );

                        close(roura[0]);
                        wait(nullptr);
                    }

                    const char html_head[] = "<!DOCTYPE html><html><body>\r\n";
                    const char html_heel[] = "</body></html>\r\n";

                    int content_size = strlen(html_head) + strlen(html_body_buf) + strlen(html_heel);
                    snprintf(l_buf, sizeof(l_buf), "%s%d%s%s%s\r\n%s", "HTTP/1.1 200 OK\r\nContent-Length: ", content_size, "\r\nContent-Type: text/html; charset=utf-8\r\n\r\n", html_head, html_body_buf, html_heel);

                    // write data to stdout
                    // int l_len = write( STDOUT_FILENO, l_buf, l_len );
                    // if ( l_len < 0 )
